...
TEST_F(NotifierTest, SendsOnlyMatchingEventType) {
    ...
    auto sub_a = notifier.subscribe<EventA>([&sum](const EventA& event) { sum += event.value; });
    auto sub_b =
        notifier.subscribe<EventB>([&other](const EventB& event) { other += event.value; });
...
TEST_F(NotifierTest, UnsubscribeIsIdempotentAndPreventsFutureCallbacks) {
    ...
    auto sub = notifier.subscribe<EventA>([&calls](const EventA&) { ++calls; });
...
TEST_F(NotifierTest, UnsubscribingOneSubscriptionKeepsSameTypeCallbacks) {
    ...
    auto first = notifier.subscribe<EventA>([&first_calls](const EventA&) { ++first_calls; });
    auto second = notifier.subscribe<EventA>([&second_calls](const EventA&) { ++second_calls; });
...
TEST_F(NotifierTest, SendUsesSnapshotAndAllowsMutationInsideCallback) {
    ...
    auto first = notifier.subscribe<EventA>([&](const EventA&) {
        ...
        late_subs.push_back(
            notifier.subscribe<EventA>([&late_calls](const EventA&) { ++late_calls; }));
    });
    second = notifier.subscribe<EventA>([&](const EventA&) { ++second_calls; });
...
TEST_F(NotifierTest, ClearDisablesOneEventType) {
    ...
    auto sub_a = notifier.subscribe<EventA>([&a_calls](const EventA&) { ++a_calls; });
    auto sub_b = notifier.subscribe<EventB>([&b_calls](const EventB&) { ++b_calls; });
    ...
    for (int i = 0; i < kThreads; ++i) {
        threads.emplace_back([&] {
            for (int j = 0; j < kIterations; ++j) {
                auto sub = notifier.subscribe<EventA>(
                    [&calls](const EventA&) { calls.fetch_add(1, std::memory_order_relaxed); });
...
TEST_F(NotifierTest, LogsWarningForCallbackOver20ms) {
    ...
    DefaultNotifier notifier;
    auto sub = notifier.subscribe<EventA>(
        [](const EventA&) { std::this_thread::sleep_for(std::chrono::milliseconds{25}); });
...
TEST_F(NotifierTest, LogsErrorForCallbackOver100ms) {
    ...
    DefaultNotifier notifier;
    auto sub = notifier.subscribe<EventA>(
        [](const EventA&) { std::this_thread::sleep_for(std::chrono::milliseconds{105}); });
